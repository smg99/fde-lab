...
program
  .name('fde-data-migration-engineer')
  .description('FDE Lab - Data Migration Engineer')
...
async function main() {
  ...
  const pyArgs = ['migrate', '--scenario', options.scenario];
  if (options.json) pyArgs.push('--json');
  if (options.manifest) pyArgs.push('--manifest');
  
  await setupAndRunPython(packageRoot, 'fde_lab.pocs.data_migration_engineer.cli', pyArgs, machineMode);
}
