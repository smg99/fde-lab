...
program
  .name('fde-solution-engineer')
  .description('FDE Lab - Solution Engineer')
...
async function main() {
  ...
  const pyArgs = ['design-solution', '--scenario', options.scenario];
  if (options.json) pyArgs.push('--json');
  if (options.manifest) pyArgs.push('--manifest');
  
  await setupAndRunPython(packageRoot, 'fde_lab.pocs.solution_engineer.cli', pyArgs, machineMode);
}
