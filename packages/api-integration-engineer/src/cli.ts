...
program
  .name('fde-api-integration-engineer')
  .description('FDE Lab - API Integration Engineer')
...
async function main() {
  ...
  const pyArgs = ['integrate-api', '--scenario', options.scenario];
  if (options.json) pyArgs.push('--json');
  if (options.manifest) pyArgs.push('--manifest');
  
  await setupAndRunPython(packageRoot, 'fde_lab.pocs.api_integration_engineer.cli', pyArgs, machineMode);
}
