...
program
  .name('fde-support-escalation-engineer')
  .description('FDE Lab - Support Escalation Engineer')
...
async function main() {
  ...
  const pyArgs = ['escalate', '--scenario', options.scenario];
  if (options.json) pyArgs.push('--json');
  if (options.manifest) pyArgs.push('--manifest');
  
  await setupAndRunPython(packageRoot, 'fde_lab.pocs.support_escalation_engineer.cli', pyArgs, machineMode);
}
