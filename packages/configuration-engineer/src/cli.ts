...
program
  .name('fde-configuration-engineer')
  .description('FDE Lab - Configuration Engineer')
...
async function main() {
  ...
  const pyArgs = ['inspect-configuration', '--scenario', options.scenario];
  if (options.json) pyArgs.push('--json');
  if (options.manifest) pyArgs.push('--manifest');
  
  await setupAndRunPython(packageRoot, 'fde_lab.pocs.configuration_engineer.cli', pyArgs, machineMode);
}
